...
Inspect `tools/list` and `get_catalog` first. The documented server exposes nine tools: `get_catalog`, `get_free_sample`, the free `audit_agent_costs`, `analyze_agent_latency`, `check_agent_quality`, the owner-authorized write tool `save_project_report`, and the sandbox `purchase_snapshot`, `purchase_changes`, `purchase_evidence`. Purchase tools receive `idempotency_key` in arguments and `Authorization: Bearer <pilot-test-key>` in the HTTP header. Obtain a pilot key through `/start`. `purchase_changes` accepts an optional `since` date. Persist a fresh ID before a new purchase; every retry of that purchase keeps the same ID, key and arguments. The server adapter sets sandbox mode internally.
